...
# спецификации на каждый объект мр Жетыбаймунайгаз
...
SAVE_FILE = './output files/Расчет цены ПНР ЖМГ.xlsx'                 # путь к папке для сохранения результата
ALIAS_FILE = './output files/Список замены ЖМГ. Ред 2.xlsx'           # путь к файлу с алиасами (заменами)
...
for file in file_list:
    ...
    if len(data_file) > 0:
        ...
        data = data_file.merge(alias_names, left_on='Тип', right_on='Тип')
